...
public static class UiTheme
{
    ...
    private static void EnsureStyles()
    {
        ...
        _panelBox = MakeBox(_panel != null ? _panel : _solidPaper, border24, 10,
            Ink, GUI.skin.label.fontSize + 1);
        _hudBox = MakeBox(_hud != null ? _hud : _solidPaper, border16, 8,
            Ink, GUI.skin.label.fontSize);
        _cardBox = MakeBox(_card != null ? _card : _solidPaper, border12, 6,
            Ink, GUI.skin.label.fontSize);

        _btnStyle = MakeButton(_btn, _btnHover, _btnActive, border12,
            Ink, GUI.skin.button.fontSize);
        _btnPrimary = MakeButton(_btnRed != null ? _btnRed : _solidRed, null, null, border12,
            Paper, GUI.skin.button.fontSize);
        ...
        _body = new GUIStyle(GUI.skin.label) { richText = true };
        _body.normal.textColor = Ink;

        _hint = new GUIStyle(GUI.skin.label) { richText = true, fontSize = GUI.skin.label.fontSize - 1 };
        _hint.normal.textColor = InkSoft;

        _field = new GUIStyle(GUI.skin.textField);
        _field.normal.background = _solidPaper;
        _field.focused.background = _solidPaper;
        _field.hover.background = _solidPaper;
        _field.active.background = _solidPaper;
